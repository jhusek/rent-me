<script lang="ts">
  ...
  import { GetDocuments, CreateDocument, DeleteDocument } from '../../wailsjs/go/main/App';
...
<Modal bind:show={showModal} title="Upload Document" size="lg">
  <form on:submit|preventDefault={saveDocument} id="document-form">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div class="col-span-full">
        <label class="label" for="filePath">File Path</label>
        <input id="filePath" class="input-field" bind:value={form.filePath} placeholder="e.g. C:\Documents\lease.pdf" />
        <p class="text-xs text-gray-400 mt-1">File picker integration coming soon.</p>
      </div>
      <div>
        <label class="label" for="fileSize">File Size (bytes)</label>
        <input id="fileSize" type="number" class="input-field" bind:value={form.fileSize} min="0" />
      </div>
      <div>
        <label class="label" for="mimeType">MIME Type</label>
        <input id="mimeType" class="input-field" bind:value={form.mimeType} placeholder="e.g. application/pdf" />
      </div>
      <div class="col-span-full">
